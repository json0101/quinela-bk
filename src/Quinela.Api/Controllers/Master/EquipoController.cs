...
namespace Quinela.Api.Controllers.Master
{
    ...
    public class EquipoController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] EquipoCreateDto dto, CancellationToken ct)
            => (await _sender.Send(new CreateEquipoCommand(dto.Nombre, dto.Confederacion, dto.Anfitrion, dto.UrlBandera, dto.EquipoIdApi, dto.EquipoIdApiLargo, dto.TorneoId, dto.Active), ct)).ToActionResult();

        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] EquipoUpdateDto dto, CancellationToken ct)
            => (await _sender.Send(new UpdateEquipoCommand(id, dto.Nombre, dto.Confederacion, dto.Anfitrion, dto.UrlBandera, dto.EquipoIdApi, dto.EquipoIdApiLargo, dto.TorneoId, dto.Active), ct)).ToActionResult();
